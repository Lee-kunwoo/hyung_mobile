document.addEventListener("DOMContentLoaded", function () {
    ...
    const modal = document.querySelector(".modal"); // modal 요소 정의 추가
    ...
    closeBtn.addEventListener("click", function () {
        modal.style.left = "-300px"; // 사이드바 닫기 위치 수정
        overlay.style.display = "none";
    });

    overlay.addEventListener("click", function () {
        modal.style.left = "-300px"; // 오버레이 클릭 시 닫힘
        overlay.style.display = "none";
    });
    ...
    document.querySelectorAll(".menu-link").forEach(link => {
        link.addEventListener("click", function (e) {
            ...
            let modalMenuList = parent.querySelector(".modalMenuList");

            if (modalMenuList.classList.contains("open")) {
                modalMenuList.classList.remove("open");
                parent.classList.remove("active");
            } else {
                document.querySelectorAll(".modalMenuList").forEach(item => {
                    item.classList.remove("open");
                });
                document.querySelectorAll(".modalMenu > li").forEach(item => {
                    item.classList.remove("active");
                });

                modalMenuList.classList.add("open");
...
/* ========= 스크롤 기반 이벤트 (PC용 wheel + Mobile 터치 이벤트) ========= */
